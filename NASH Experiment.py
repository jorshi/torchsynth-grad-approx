...
synth_config = SynthConfig(
    buffer_size_seconds=len(target) / sample_rate,
...
def fitness(individuals):
    ...
    new_values = torch.stack(
        [torch.tensor(list(ind)) for ind in individuals], dim=1
    ).to("cuda")
...
parameters = [param for _, param in sorted(voice.named_parameters())]
new_values = torch.stack(
    [torch.tensor(list(ind)) for ind in tools.selBest(pop, BATCH_SIZE)], dim=1
).to("cuda")
for i, parameter in enumerate(parameters):
    parameter.data = new_values[i]
